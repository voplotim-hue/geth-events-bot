...
- Роль пользователя в карточке: `Участник`, `Помощник`, `Админ`, `Гость`.
- Автоматическая пометка `Гость`, если в анкете указана церковь не `Гефсимания`; гости не участвуют в поздравлениях с днем рождения.
...
- Субботние опросы присутствия в чате подростков и чате лидеров.
- Ручная отметка фактического присутствия, настройка нагрузки лидеров и случайное распределение групп.
- Закрытые заметки после беседы в отдельной таблице душепопечения.
...
После включения `WEEKLY_SERVICE_ENABLED=true` бот каждую субботу в `WEEKLY_SERVICE_POLL_TIME` по часовому поясу `TIMEZONE` отправляет два опроса: один в чат лидеров, второй в чат подростков. По умолчанию публикация выключена, чтобы опросы не появились без явного запуска.

Управляющий из `WEEKLY_SERVICE_COORDINATOR_IDS` видит в личном чате кнопку `Субботнее служение` и может отменить опрос ближайшей субботы. Отмена снимает кнопки с уже опубликованных опросов и блокирует их автоматическую публикацию, если она ещё не произошла.

Фактическое присутствие и нагрузка лидера отмечаются на листе `Посещаемость служений`: `Да` в колонке `Фактически присутствует` и вес `1`–`4` в колонке `Нагрузка лидера`. После команды «Сформировать группы» бот распределяет присутствующих подростков случайно, но пропорционально весам лидеров.

Заметки лидеров после беседы сохраняются в отдельном файле `GethTeens — Душепопечение`. В Google Drive доступ к нему назначается по Gmail-адресам, поэтому для Романа и Александра понадобятся их адреса Google. Telegram ID для этого недостаточно.
...
- `Настроить русский вид таблицы`
- `Обновить порядок и цвета ролей`

Меню может проставить русские заголовки, добавить колонку `Роль`, поменять значение роли для выделенных строк, отсортировать анкеты и раскрасить строки:

- `Админ` - сверху, светло-красное выделение.
- `Помощник` - после админов, светло-желтое выделение.
- `Участник` - ниже, без яркого выделения.
- `Гость` - ниже участников, серое выделение; не попадает в ежедневные поздравления.
